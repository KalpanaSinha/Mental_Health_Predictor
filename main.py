...
def chart1():
    ...
    fig = ff.create_distplot(hist_data1,group_labels)
    graphJSON = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
 
    return render_template('explore.html', graphJSON=graphJSON)

@main.route('/chart2')
def chart2():
    df = showExplorePage()
    fig= px.histogram(df, x='treatment', color="Gender", barmode='group')
    graphJSON = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
    return render_template('explore.html', graphJSON=graphJSON)
...
def chart3():
    df = showExplorePage()
    fig= px.histogram(df, x='age_range', color="Gender", barmode='group')
    graphJSON = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
    return render_template('explore.html', graphJSON=graphJSON)
